...
- `separate_private_network_interface` (Boolean) This attribute can only be set if your server also has a `vpc_id` attribute set. When enabled, the server will use a separate network interface for private (VPC) traffic, allowing the server to have both a public IPv6 address and private VPC connectivity on separate NICs. This is useful when you want to use a VPC and also have a public IPv6 address, or when you want public and private traffic on different network interfaces.
